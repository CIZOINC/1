class UserCustomValidator < ActiveModel::Validator
  ...
  def validate(record)
      ...
      record.errors[:codes] << "422.3"  if (valid_email? && email_has_been_taken? && new_record?)
      #password
      ...
      record.errors[:codes] << "422.7"  if (!blank_password? && blank_password_confirmation? && password_is_valid?)
  ...
  %w(email password birthday password_confirmation).each do |param|
